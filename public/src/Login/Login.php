...
class Login
{
    ...
    private function checkUserInfo()
    {
        if (!$this->getResult()) {
            ...
            $read->exeRead(PRE . "usuarios", "WHERE password = :pass AND (login_social = " . $socialUser . ")", "pass={$this->senha}", !0, !0, !0);
            if ($read->getResult()) {
    ...
    private function attemptExceded()
    {
        ...
        $read = new Read();
        $read->exeRead(PRE . "login_attempt", "WHERE data > DATE_SUB(NOW(), INTERVAL 15 MINUTE) && ip = '{$ip}' && email = '{$this->user}'", !0, !0, !0);
