...
if (defined("FACEBOOKAPLICATIONID") && !empty(FACEBOOKAPLICATIONID) && defined('FACEBOOKENTITY') && !empty(FACEBOOKENTITY)) {
    if (file_exists(PATH_HOME . "entity/cache/" . FACEBOOKENTITY . ".json")) {
        if (!SERVICEWORKER && HOME === "" && HOME !== SERVER) {
            echo "<iframe src='" . SERVER . "loginBtnFacebook' frameborder='0' scrolling='no' id='faceLoginFrame' style='width: 165px;border: 0;height: 45px; float: right;border-radius: 3px;'></iframe>";
        } else {
            ?>
            <script>
